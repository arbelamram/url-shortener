// client/src/components/CopyButton.js
// Reusable copy-to-clipboard button: copies provided text and shows brief success/failure feedback.
// Uses Clipboard API when available and falls back to execCommand("copy") for older environments.
 ...
 * - timeoutMs: number - how long to show feedback before resetting
 */
export default function CopyButton({
  ...
  /**
   * Cleanup timer on unmount to avoid setState after component unmount.
   */
  useEffect(() => {
    return () => {
  ...
  /**
   * Schedules a reset back to idle state after timeoutMs.
   */
  const resetLater = () => {
    if (timerRef.current) clearTimeout(timerRef.current);
   ...
   * - Falls back to execCommand if Clipboard API isn't available
  ...
  // Tooltip text is announced for both success and failure (a11y)
  const liveText = isCopied ? 'Copied' : isError ? 'Failed to copy' : '';
  ...
  return (
    <span className={`copy-wrap ${className}`}>
      ...
      {/* Tooltip / live region: announces copy result briefly */}
      <span
        className={`copy-tip ${isCopied || isError ? 'show' : ''}`}
        role='status'
        aria-live='polite'
      >
        {liveText}
